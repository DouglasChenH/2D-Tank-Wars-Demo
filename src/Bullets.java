...
public class Bullets {
	...
	private static Map<String, Image> imgs = new HashMap<String, Image>(); // map specific bullet image to each direction  
	...
	private void move() {
		...
		case R:
			x += speedX; // bullet moves to right
			break;
	...
	public boolean hitTank(Tank t) { 
		...
		if (this.live && this.getRect().intersects(t.getRect()) && t.isLive()
			...
			if (t.isGood()) {
				...
				if (t.getLife() <= 0)
					t.setLive(false); // health <0 then die
			} else {
				t.setLive(false); 
			}
	...
	public boolean hitWall(MetalWall w) { // bullet hits on metalWall
		if (this.live && this.getRect().intersects(w.getRect())) {
			this.live = false;
			//this.tc.metalWall.remove(w); //bullet can't pass by metal wall 
			return true;
		}
		return false;
	}

	public boolean hitHome() { // bullet hits on home
		if (this.live && this.getRect().intersects(tc.home.getRect())) {
			this.live = false;
			this.tc.home.setLive(false); // set home to death after one bullet
			return true;
		}
		return false;
	}
